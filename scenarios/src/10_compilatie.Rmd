...
tussenstap3_sc1 <- ifel(sc1_ecosysteem_dedrain == 220, 220, tussenstap2_sc1)
...
# Cellen met infiltratiemaatregelen krijgen code 210
...
test <- ifel(sc1_ecosysteem_infilt %in% c(201, 202, 203, 204, 205, 210, 220) & !is.na(kle_recl_sc1), 
             )




tussenstap4_sc1 <- "nog in te vullen"
...
# Wat doen we met de KLE-cellen?
# Optie 1: alle cellen met KLE worden aan een nieuwe code toegewezen met aparte kc- en CV-waarden -> input in Invest
# Optie 2: we berekenen manueel wat het effect is (los van Invest)
```

#### 4. Onthardingscellen invoegen
...
sc1_ecosysteem_finaal <- ifel(tussenstap4_sc1 %in% c(101, 102) & !sc1_ecosysteem_ontharding %in% c(101, 102), sc1_ecosysteem_ontharding, tussenstap4_sc1)
```
...
test <- ifel(sc2_ecosysteem_infilt == 210, tussenstap3_sc2, NA)
...
sc2_ecosysteem_finaal <- ifel(tussenstap4_sc2 %in% c(101, 102) & !sc2_ecosysteem_ontharding %in% c(101, 102), sc2_ecosysteem_ontharding, tussenstap4_sc2)
```
...
sc3_dedrain_cellen <- rast(str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/output/sc3_dedrain_cellen.tif"))
...
tussenstap3_sc3 <- rast(str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/output/tussenstap3_sc3.tif"))

sc3_ecosysteem_infilt <- rast(str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/output/sc3_ecosysteem_infilt.tif"))


sc3_ecosysteem_infilt # in sc1 en 2 blijven dit akkers / in sc3 is er een versie waar het akkers blijven (75%-scenario) en een versie waar de cellen van sc2 veranderen in grasland/bos

test <- ifel(sc3_ecosysteem_infilt == 210, tussenstap3_sc3, NA)
...
tussenstap4_sc3 <- rast(str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/output/tussenstap4_sc3.tif"))

# verhardingscellen (code 101 en 102) worden vervangen door de ontharde cellen

sc3_ecosysteem_finaal <- ifel(tussenstap4_sc3 %in% c(101, 102) & !sc3_ecosysteem_ontharding %in% c(101, 102), sc3_ecosysteem_ontharding, tussenstap4_sc3)
```
