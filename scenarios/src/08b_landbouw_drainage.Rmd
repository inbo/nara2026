...
- vernattingscontour_sc2.tif -\> vernattingscontouren voor toekomstkaart en veenherstel scenario 2 (zie script `x6_vernattingscontouren`)
...
#### b) Omzetting toekomstlaag naar ecosysteemkaart
...
# 1. Definieer de uitzonderingen
uitzonderingen <- 
  ecosysteem_2022 %in% c(101, 102, 801, 802) |
  is.na(toekomst_eco) |
  toekomst_eco == 0 | # 0 = geen habitat
...
De doeloppervlakte (`sc2_doel` = `sc1_doel`) wordt geplaatst in het afstroomgebied van natuur, op plaatsen waar er natte natuur bijkomt volgens scenario 2 (`natuur_ecosysteem_sc2` -\> zie script `04_IHD`) en waar er door die bijkomende natuur vernatting optreedt (vernattingscontour_sc2 -\> zie script `x6_vernattingscontouren`).

Welke ecosysteemcodes komen in aanmerking voor dedrainage? -\> landbouw (2xx, 301) en onbepaald grasland (300). Bossen en semi-natuurlijke graslanden -\> daarvoor zou de ecosysteemkaart ingedeeld moeten worden in droge en natte graslanden en bossen. We laten die cellen buiten beschouwing omdat een eventuele vernatting op die cellen deel uitmaakt van het natuurbeheer. Voor dedrainage kijken we naar extra maatregelen bovenop natuurbeheer.

Cellen met nieuwe natte natuur worden eerst opgevuld (score 100), vervolgens de geschikte cellen in het voedingsgebied (score 10) en tenslotte de rest van de geschikte cellen (2-4).

- Nieuwe natte natuur (0.5 en 0 m mv) -\> alle cellen dedraineren (glg_max = gangbare drainagediepte = minstens 0.75 m)
  - codes 702, 701 en 420 -\> grondwater tot tegen het maaiveld
  - codes 320 en 410 -\> grondwater 0.5 m onder het maaiveld
- Vernattingscontouren (0 - 0.25 - 0.5 - 0.75 en 1 m mv) -\> differentiëren ifv ecosysteem:
  - akkers -\> overal dedraineren (in zones met code 1, 2, 3 en 4)
  - grasland -\> alleen dedraineren als glg \> 0.75 m (in zones met code 1 , 2 en 3)
...
natte_natuur[natte_natuur > 0] <- 1
natte_natuur[natte_natuur == 0] <- NA

voedingsgebied_combi[voedingsgebied_combi > 0] <- 1

vernattingscontour_sc2

geschikt_sc2 <- ifel(!is.na(voedingsgebied_combi) & !is.na(natte_natuur), 110 + potentie_dedrain, ifel(!is.na(voedingsgebied_combi), 10 + potentie_dedrain, ifel(!is.na(potentie_dedrain), potentie_dedrain, NA)))
```
...
Akker blijft akker -\> grondwaterstijging +0.5m

natte natuur -\> stijging tot tegen maaiveld of 0.5 m onder maaiveld
