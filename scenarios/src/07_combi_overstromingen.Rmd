...
targets <- c("ecosysteem_2022.tif", "toekomstlaag.shp", "veen1970_poly.shp", "wegen.shp", "overstromingsdiepte_t100_flu_plu.tif", "dhm.tif", "vha.shp", "overstromingstolerantie_habitats_gis.xlsx", "grondwaterregime_habitats.xlsx")
...
# clusters < 50 cellen verwijderen
f <- freq(clusters)
clusters_clean <- clusters
clusters_clean[clusters_clean %in% f$value[f$count < 50]] <- NA
...
writeRaster(overstromingsvrije_clusters, filename = str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/data/intolerant_zone_bever.tif"), overwrite = TRUE)
...
combineerbaar_vernatting_t100 <- mask(combineerbaar_vernatting, vernattingszone)

# Resultaat opslaan (1 = niet combineerbaar)
writeRaster(combineerbaar_vernatting_t100, filename = str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/data/combineerbaar_vernatting_t100.tif"), overwrite = TRUE)
```

### 3.2. Zones vrijwaren van vernatting

De aanpak is dezelfde als die voor overstromingen.
...
waterdiepte <- crop(waterdiepte, ecosysteem_2022)
overstroombaar_barriere <- ifel(waterdiepte > 0, 0, NA)
...
# Verrasteren
wegen_barriere <- rasterize(wegen_gebufferd, ecosysteem, touches = TRUE, field = NA, background = 0)
wegen_barriere <- crop(wegen_barriere, ecosysteem_2022)
...
vernattingsvrije_clusters <- ifel(!is.na(eindkaart_min_afstand), 1, NA)
...
Als ruwe benadering schatten we de invloedssfeer in op basis van een logaritmisch verband tussen de peilverlaging en een maximale afstand (100 m).
...
for (h0 in stappen) {
  ...
  # Controleer of er daadwerkelijk percelen zijn met deze waarde
  if (global(bron_cellen, "max", na.rm = TRUE)[1, 1] == 1) {
...
writeRaster(invloedszone, filename = str_c("C:/R/NARA2026/nara2026-git/scenarios/", studiegebied, "/data/intolerant_zone_veenherstel.tif"), overwrite = TRUE)

# mapview(invloedszone, col.regions = "lightblue", na.color = "transparent") + mapview(r_veen, col.regions = "brown3", na.color = "transparent")
```
