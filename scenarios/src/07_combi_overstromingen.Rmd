...
- wegen.shp -\> wegen als barriere voor Costdist-analyse
...
# wegen toevoegen als barriere

# 1. Laad je vectorlijnen en het referenteraster (bijv. het DHM)
wegen_vector <- vect("wegen.shp")

# 2. Bereken de celgrootte en de benodigde buffer
buffer_breedte <- sqrt(celgrootte^2 + celgrootte^2) / 2

# 3. Buffer de wegen
wegen_gebufferd <- buffer(vect(wegen), width = buffer_breedte)

# 4. Verraster de gebufferde wegen
# touches = TRUE, field = NA (barrière), background = 0 (begaanbaar)
wegen_barriere <- rasterize(wegen_gebufferd, ecosysteem, touches = TRUE, field = NA, background = 0)

# Wegen toevoegen aan barrières
mask <- ifel(is.na(mask) | is.na(wegen_barriere), NA, 0)


# clusters opkuisen door rivieren uit te knippen en clusters < 10 cellen te verwijderen
...
# cluster_stats <- zonal(waterdiepte, clusters_clean, fun = "min", na.rm = TRUE)
# Of een percentiel (p10 = 10% ondiepste cellen uitsluiten)
...
mapview(overstromingsvrije_clusters, na.color = "transparent") + mapview(clusters_clean, na.color = "transparent") + mapview(wegen)
